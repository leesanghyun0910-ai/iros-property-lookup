...
주소를 대량으로 입력하면 **부동산고유번호(14자리)**를 일괄 조회하고, 인터넷등기소 **일괄열람 등록양식(.xls)**을 자동 생성하는 웹 서비스. 토지·건물 관련 가격과 이용계획, 건축물대장·등기부등본 자료도 함께 조회해 PDF/XLSX로 묶어준다.

세무·법무 실무에서 주소를 하나씩 검색하던 반복 작업을 없애는 것이 목표다. 일괄열람 양식은 사용자가 인터넷등기소에서 직접 처리할 수 있고, 앱 안에서는 선불전자지급수단으로 건당 700원의 등기부등본 열람도 지원한다.
...
- **등기부등본** — 토지·건물·집합건물의 인터넷등기소 열람 PDF를 개별 또는 최대 30건씩 병합하고 동일 고유번호는 캐시 재사용. 캐시 미스는 건당 700원 과금
...
| 등기부등본 | 인터넷등기소 (KT 디지털융합플랫폼·하이픈 대행) | 고유번호 기반 유료 열람 PDF |
...
Cloudflare Worker (정적 자산 + /api/*)
   React SPA (입력·검토·내보내기)
        ...
        ├─ /api/property-register/download → 인터넷등기소 등기부등본 유료 열람·PDF 병합
        └─ /api/pnu        → 주소 → PNU
                              │
                     [법정동코드 캐시] Cloudflare KV

건축물·토지대장·등기부등본 작업 상태·메타 ─ Cloudflare D1
건축물·토지대장·등기부등본 임시 PDF       ─ Cloudflare R2 (만료 후 정리)
```
...
- 등기부등본은 다운로드 시점에만 인터넷등기소 열람을 요청한다. 원본은 14자리 고유번호별로 D1/R2에 캐시하며 요청당 30건·동시 2건으로 제한하고 실패 호출은 자동 재시도하지 않는다.
...
| `POST /api/property-register/download` | `{items:[{key,uniqNo,address?,pinFmt?}]}` → 인터넷등기소 등기부등본 PDF 유료 열람·병합 |
...
IROS_ID=...           # 인터넷등기소 회원 아이디
IROS_PW=...           # 인터넷등기소 회원 비밀번호
MON_NO1=...           # 선불전자지급수단번호 앞 8자리
MON_NO2=...           # 선불전자지급수단번호 뒤 4자리
MON_PW=...            # 선불전자지급 비밀번호
```
...
npx wrangler d1 execute iros-building-register --local --file=./migrations/0003_property_register_cache.sql
...
npx wrangler secret put IROS_ID
npx wrangler secret put IROS_PW
npx wrangler secret put MON_NO1
npx wrangler secret put MON_NO2
npx wrangler secret put MON_PW
npx wrangler secret put ADMIN_TOKEN
```

배포에는 KV 네임스페이스(`LDONG`), 건축물·토지대장·등기부등본이 함께 쓰는 D1/R2, Cron이 `wrangler.toml`에 설정돼 있다. 신규 환경에는 `migrations/0001_building_register_cache.sql`, `0002_land_register_cache.sql`, `0003_property_register_cache.sql`을 순서대로 적용한다. 상가/오피스 기준시가는 별도 적재 없이 Hometax를 실시간 조회하며, 지번 주소는 KV의 법정동코드로 도로명 검색 단계를 생략한다.
...
- **인터넷등기소 열람 과금**: 캐시가 없는 등기부등본은 선불전자지급수단에서 건당 700원이 차감된다. 요청당 최대 30건·동시 2건으로 제한하고 실패 호출은 자동 재시도하지 않는다.
